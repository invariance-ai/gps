...
- [Why gps](#why-gps)
- [The core loop in action](#the-core-loop-in-action)
- [Commands](#commands)
- [Agent integration](#agent-integration)
- [Capture & promotion policy](#capture--promotion-policy)
- [Concepts: notes, invariants, decisions](#concepts-notes-invariants-decisions)
- [How it compares](#how-it-compares)
- [Benchmarks](#benchmarks)
- [Experimental: live docs](#experimental-live-docs)
...
npx -y @invariance/gps setup --yes --with-cursor   # Cursor
```

Install several integrations at once: `npx -y @invariance/gps setup --yes --with-claude --with-codex --with-cursor`. Prefer a global install? `npm install -g @invariance/gps && gps setup --yes --with-claude`.
...
Optional starter packs are explicit because they depend on your stack:

```bash
gps invariant init --stack stripe
```

Want the long version? See [`docs/guide/getting-started.md`](docs/guide/getting-started.md) for the 10-minute walkthrough and [`docs/guide/commands.md`](docs/guide/commands.md) for the full CLI reference. Launching or evaluating the project? [`docs/launch-audit.md`](docs/launch-audit.md) has the MVP bar and citable benchmark claims; [`docs/product-hunt.md`](docs/product-hunt.md) has launch copy. Security/privacy review? [`SECURITY.md`](SECURITY.md) documents what GPS stores, what hooks/MCP can run, and how to disable integrations.
...
The launch demo shows one thing clearly: an agent makes or nearly makes a repo-specific mistake, GPS captures the correction, and the next edit sees it before touching code.
...
A full brief carries structure, tests, invariants, notes, decisions, and risk in one shot:
...
The rule of thumb: if finding it took real search, record it once so the next agent does not repeat the search loop.

## Commands

The whole happy path is `setup` once, then the core loop above. Everything below is the larger surface for power users and automation. Run `gps --help` for the curated launch surface, `gps <command> --help` for any advanced command, and see [`docs/guide/commands.md`](docs/guide/commands.md) for the full generated reference.
...
gps recall "<query>"                                  # search all memory by topic
gps prime                                             # compact session-start standing context
```
...
`gps serve --observe` records *which symbol was queried* and *when*, into `.gps/observations.json`. **Nothing else** — no tool arguments beyond the symbol name, no tool results, no conversation content. The privacy line: gps never persists what an agent asked or what it received, only that `createRefund` was looked at 6 times this week. `gps suggest` reads those counts and surfaces the symbols agents touch a lot that have no covering invariant — the agent's repeated confusion becomes the **authoring queue**. To try automatic nudges in Claude Code, reinstall with `gps install claude --auto-suggest`; the hook runs `gps suggest --auto`, silent unless `.gps/config.yml` has `auto_suggest: true`.
...
```bash
gps postmortem --pr 1287                              # propose an invariant from a regression PR (prints a native-agent prompt)
gps postmortem --diff-file my.diff --symbol createRefund   # offline alternative
gps promote createRefund                             # find note clusters that should become invariants (rule-based, no API key)
gps attach --transcript path/to/transcript.txt --symbol createRefund --session "PR-1287"   # distill a transcript into Decisions
gps pr-intent --pr 1287                              # extract Decisions from a PR's description, reviews, and comments
```
...
GPS is CLI-first because coding agents already know how to run shell commands. Treat `gps` like `rg`: a local command the agent runs before and after edits. The installer also writes the native files each agent reads at startup. Per-agent install guides: [`Claude Code`](docs/guide/agents/claude.md), [`Codex CLI`](docs/guide/agents/codex.md), [`Cursor`](docs/guide/agents/cursor.md).

| Agent | What GPS writes | How the agent learns |
|---|---|---|
| **Claude Code** | `CLAUDE.md`, `.claude/skills/gps/SKILL.md`, `.claude/settings.json`, `.mcp.json` | The skill and `CLAUDE.md` teach the workflow. Hooks inject context, refresh the index, and capture lessons. MCP exposes `mcp__gps__prepare_edit`. |
| **Codex CLI** | `AGENTS.md`, `.codex/config.toml` | `AGENTS.md` teaches Codex to run `gps prepare` / `gps brief`. MCP exposes structured GPS tools. `notify` captures durable preferences after each turn. |
| **Cursor** | `.cursor/rules/gps.mdc`, `.cursor/mcp.json` | The always-attached rule teaches the workflow. MCP tools are available for context and explicit memory capture. |

(Install commands are in [Quickstart](#quickstart).)

**Claude Code** wires five non-blocking hooks: `SessionStart` (rebuilds the index, prints standing preferences), `UserPromptSubmit` (auto-loads context for symbols named in your prompt), `PreToolUse` Edit/Write (refreshes the index), `PostToolUse` Bash (records failures against the last-prepared symbol), and `Stop` (distills the session into Decisions). The skill tells Claude to:
...
If you only want the portable Claude Code skill, the important file is `.claude/skills/gps/SKILL.md` — that alone teaches Claude the loop. The one-command setup writes it for you and adds hooks/MCP so the behavior is automatic. Inspect what Claude sees with `cat CLAUDE.md` and `cat .claude/skills/gps/SKILL.md`.

**Codex CLI** writes `AGENTS.md` instructions, registers `gps serve` as an MCP server, and configures a `notify` hook that distills each turn — auto preference and directive capture happen via this hook.
...
### Want Claude Code to install GPS for you?

Paste this into a Claude Code session at the root of your repo:
...
- **`--capture`** — where freshly captured memory lands. `auto` (default) persists it live. `inbox` queues it for human approval via `gps inbox` before anything activates.
...
- **`--auto-suggest`** — default off. When enabled, supported hooks may run `gps suggest --auto` at turn end and print a short authoring queue: hot or failure-prone symbols that still lack notes/invariants, plus lessons near promotion. It configures MCP as `gps serve --observe` so the queue has metadata. It never writes memory by itself.

Defaults (`capture=auto`, `promote=safe`, `auto_suggest=false`) make memory useful immediately while still gating risky topics. If you choose `capture=inbox`, promotion defaults to manual because humans are already reviewing memory before activation. Run `gps inbox` to review queued captures, and `gps promote --auto` to apply the promotion policy (add `--dry-run` to preview); override for a single run with `gps promote --auto=safe` (or `=never` / `=all`).

### The reviewed path, end to end
...
Corrections about test coverage are treated as first-class memory. If a user says "No, bad Codex, you need to write more tests" after a prepared edit, the Stop/notify capture path records a `user-correction` note on the last prepared symbol even when no LLM API key is configured. Future `gps prepare <symbol>` calls then surface that testing correction before the next edit.

## Concepts: notes, invariants, decisions
...
To opt in persistently for a repo, set `experimental.live_docs: true` in `.gps/config.yml`. The server binds to `127.0.0.1` by default, regenerates on refresh, and is intended for local preview only while the UX settles.
...
What gps is **not (yet)**, honestly and up front:

- **Parser precision is ~90% on typical code**, lower on decorators-as-factories, dynamic dispatch, and heavy macros. The default backend is tree-sitter (WASM, zero native deps) for TS/JS/Python with body/`end_line` tracking; a regex backend (8 languages) is the fallback. Trade-off documented in [`packages/core/src/parser.ts`](packages/core/src/parser.ts). LSP-backed reference resolution is the next accuracy step.
- **No semantic import resolution.** Re-exports and barrel files may miss call edges.
- **No cross-repo / monorepo-aware symbol IDs** yet — each repo is its own graph.
- **Proof base is n=1.** The dogfood quality result above is one repo, 10 prompts. We're running this against more repos next; see [`docs/dogfood-runbook.md`](docs/dogfood-runbook.md) and contribute a result.
